...
## Same-host path validation

After the final routing changes, the running preview returned HTTP 200 for `/landing-page/`, `/landing-page/privacy.html`, `/landing-page/terms.html`, and `/app`. The landing page rendered with subpath-correct asset URLs such as `/landing-page/logo.svg` and its `Open the app` CTA navigated to `/app`. The authenticated app rendered its `Learn more about DropWatch` link as `/landing-page/`, and clicking it returned to the same-host landing page. The root build also completed successfully after building and mounting the landing-page bundle at `dist/public/landing-page`.
