...
The landing page can continue to be deployed independently from `landing-page/`, while the root application can continue using the managed DropWatch deployment. A future canonical GitHub repository can contain both directories without requiring the landing page to be removed or the app to be forced into the landing page’s Vercel configuration.
...
4. Replace or relocate the landing-page root configuration deliberately; do not merge two competing `package.json`, Vite, TypeScript, or deployment entry points accidentally.
...
8. Deploy the migration branch to a preview environment. Run the full test suite, type check, production build, authenticated smoke test, provider no-match test, notification unsubscribe test, and schedule-readiness check.
